var twiliochat = (function() {
    ...
    function handleInputTextKeypress(event) {
        if (event.keyCode === 13) {
            ...
            var dataMessage = {username:DBusername,channel:tc.currentChannel.state.friendlyName,message:$(this).val()};
            $.ajax({
                data: dataMessage,
                type: "post",
                url: "/saveMessage",
                success: function(data){
                    // data upon success reads "saved to database"
                }
            });
    ...
    function connectClientWithUsername() {
        ...
        this.DBusername = $usernameInput.val();
        $usernameInput.val('');
        if (usernameText == '') {
